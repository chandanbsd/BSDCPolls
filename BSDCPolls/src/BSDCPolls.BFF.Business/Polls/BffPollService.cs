...
public sealed class BffPollService : IBffPollService
{
    ...
    /// <inheritdoc />
    public async Task<PollDetailResponse> CreateAsync(CreatePollRequest request, string bearerToken, CancellationToken ct = default)
    {
    ...
    /// <inheritdoc />
    public async Task<PollDetailResponse> GetByUidAsync(Guid pollUid, string bearerToken, CancellationToken ct = default)
    {
        var client = CreateAuthenticatedClient(bearerToken);
    ...
    /// <inheritdoc />
    public async Task<PollFeedResponse> GetFeedAsync(PollStatus? status, int page, int pageSize, string bearerToken, CancellationToken ct = default)
    {
    ...
    /// <inheritdoc />
    public async Task<PollDetailResponse> ChangeStatusAsync(Guid pollUid, ChangePollStatusRequest request, string bearerToken, CancellationToken ct = default)
    {
        var client = CreateAuthenticatedClient(bearerToken);
        var response = await client.PatchAsJsonAsync($"/api/internal/polls/{pollUid}/status", request, ct);
        await EnsureSuccessAsync(response, "Change poll status", ct);
        return (await response.Content.ReadFromJsonAsync<PollDetailResponse>(ct))!;
    }

    /// <inheritdoc />
    public async Task<PollQuestionResponse> AddQuestionAsync(Guid pollUid, AddPollQuestionRequest request, string bearerToken, CancellationToken ct = default)
    {
        var client = CreateAuthenticatedClient(bearerToken);
        var response = await client.PostAsJsonAsync($"/api/internal/polls/{pollUid}/questions", request, ct);
    ...
    /// <inheritdoc />
    public async Task<PollQuestionResponse> PushQuestionAsync(Guid pollUid, Guid questionUid, string bearerToken, CancellationToken ct = default)
    {
        var client = CreateAuthenticatedClient(bearerToken);
        var response = await client.PostAsync($"/api/internal/polls/{pollUid}/questions/{questionUid}/push", null, ct);
        await EnsureSuccessAsync(response, "Push poll question", ct);
        return (await response.Content.ReadFromJsonAsync<PollQuestionResponse>(ct))!;
    }

    /// <inheritdoc />
    public async Task<PollResultsResponse> GetResultsAsync(Guid pollUid, string bearerToken, CancellationToken ct = default)
    {
        var client = CreateAuthenticatedClient(bearerToken);
