...
public sealed class PollRepository : IPollRepository
{
    ...
    /// <inheritdoc />
    public Task<Poll?> GetByUidAsync(Guid uid, int? requestingUserId, CancellationToken ct = default) =>
        _db.Polls
        ...
        CancellationToken ct = default)
    {
        var query = _db.Polls
            ...
            .Where(p => p.IsActive && (
                ...
                || _db.Invitations.Any(i => i.PollId == p.Id && i.InviteeId == userId && i.IsActive)));
    ...
    /// <inheritdoc />
    public Task<Poll?> GetWithSubmissionsAsync(Guid pollUid, int creatorId, CancellationToken ct = default) =>
        _db.Polls
            .Include(p => p.Questions)
                .ThenInclude(q => q.AnswerOptions)
            .Include(p => p.Questions)
                .ThenInclude(q => q.Submissions)
            .FirstOrDefaultAsync(p => p.Uid == pollUid && p.CreatorId == creatorId && p.IsActive, ct);
