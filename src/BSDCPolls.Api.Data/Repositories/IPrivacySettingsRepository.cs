...
public interface IPrivacySettingsRepository
{
    ...
    Task<UserPrivacySettings> CreateAsync(
    ...
    Task<IReadOnlyList<InviteAllowlistEntry>> GetAllowlistAsync(
        int ownerId,
        CancellationToken ct = default
    );

    /// <summary>Returns the allowlist entry matching the owner/allowed-user pair, or <c>null</c>.</summary>
    Task<InviteAllowlistEntry?> GetAllowlistEntryAsync(
        int ownerId,
        int allowedUserId,
        CancellationToken ct = default
    );
    ...
    Task<InviteAllowlistEntry> AddAllowlistEntryAsync(
    ...
    Task<InviteAllowlistEntry?> GetAllowlistEntryByUidAsync(
        int ownerId,
        Guid allowedUserUid,
        CancellationToken ct = default
    );
